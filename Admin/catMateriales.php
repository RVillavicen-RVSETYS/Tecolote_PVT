...
<html lang="es">
	...
	<body class="menubar-hoverable header-fixed me">
		...
		<div id="base">
								...
								<!-- BEGIN ACTION -->
										<div class="card card-bordered style-primary">
											<div class="card-head">
												...
												<header><i class="fa fa-fw md-view-module"></i> Materiales</header>
											</div><!--end .card-head -->
											<div class="card-body style-default-bright">
												<?php
													require('../include/connect.php');
													$sql="SELECT *
																FROM catmateriales bc
																ORDER BY nombre ASC";
													...
													<tbody>
														...
														while ($dat = mysqli_fetch_array($res)) {
															...
															<tr class="">
																...
																<td class="text-right">
																	<button type="button" class="btn btn-icon-toggle" onclick="formEditMateriales('.$dat['id'].')" data-toggle="modal" data-target="#formEditMateriales"><i class="fa fa-pencil" data-toggle="tooltip" data-placement="top" data-original-title="Editar Registro"></i></button>
																</td>
															</tr>';
														}
														?>
												...
												<div class="col-lg-12">
													<form class="form" role="form" action="../funciones/registraNuevoCatMateriales.php" method="post">
														<div class="form-group floating-label col-lg-8">
															<input type="text" class="form-control" id="newMateriales" name="nombre">
															<label for="newMateriales">Nombre del Material</label>
														</div>
														<div class="col-lg-4 text-center">
															<button type="submit" class="btn ink-reaction btn-raised btn-primary btn-loading-state" data-loading-text="<i class='fa fa-spinner fa-spin'></i> Cargando...">Agregar Material</button>
														</div>
													</form>
												</div>
											</div><!--end .card-body -->
			...
			<div class="modal fade" id="formEditMateriales" tabindex="-1" role="dialog" aria-labelledby="formEditMaterialesLabel" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content" id="editaMaterialesContent">
					</div><!-- /.modal-content -->
				</div><!-- /.modal-dialog -->
			</div><!-- /.modal -->
		...
		$( document ).ready(function() {
			<?php
			if (isset( $_SESSION['ATZmsjAdminMateriales'])) {
				echo "notificaBad('".$_SESSION['ATZmsjAdminMateriales']."');";
				unset($_SESSION['ATZmsjAdminMateriales']);
			}
			if (isset( $_SESSION['ATZmsjSuccesAdminMateriales'])) {
				echo "notificaSuc('".$_SESSION['ATZmsjSuccesAdminMateriales']."');";
				unset($_SESSION['ATZmsjSuccesAdminMateriales']);
			}
			?>
		});

		function formEditMateriales(ident){
			$("#editaMaterialesBody").html('<span class="text-default"><img src="../assets/img/loading.gif" class="img-responsive" style="margin-bottom:-1em" alt="Cargando..."/></span>');
			//alert('ID: '+ident);
			$.post("../funciones/formEditaMateriales.php",
				{ ident:ident, pag:'<?=$info->pagina;?>' },
					function(respuesta){
						$("#editaMaterialesContent").html(respuesta);
					});
		}
