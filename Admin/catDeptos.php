...
<html lang="es">
	...
	<body class="menubar-hoverable header-fixed me">
								...
								<!-- BEGIN ACTION -->
										<div class="card card-bordered style-primary">
											<div class="card-head">
												...
												<header><i class="fa fa-fw md-business"></i> Departamentos</header>
											</div><!--end .card-head -->
											<div class="card-body style-default-bright">
													...
													<tbody>
														...
														while ($dat = mysqli_fetch_array($res)) {
															...
															if ($dat['estatus'] == 0) {
																...
																$estatusColor = '';
																$estatus = '<i class="fa fa-close">';
															}else {
																$txtColor = '';
																$estatusColor = 'text-success';
																$estatus = '<i class="fa fa-check">';
															}
															...
															<tr class="'.$txtColor.'">
																<td>'.$count.'</td>
																<td>'.$dat['nombre'].'</td>
																<td class="col-lg-1 text-center '.$estatusColor.'">'.$estatus.'</td>
																<td class="text-right">
																	<button type="button" class="btn btn-icon-toggle" onclick="formEditDepto('.$dat['id'].')" data-toggle="modal" data-target="#formEditDepto"><i class="fa fa-pencil" data-toggle="tooltip" data-placement="top" data-original-title="Editar Registro"></i></button>
																</td>
															</tr>';
														}
														?>
												...
												<form class="form" role="form" method="post" action="../funciones/registraNuevoCatDepto.php">
													<div class="form-group floating-label col-lg-8">
														<input type="text" class="form-control" id="newDepto" name="newDepto" required>
														<label for="regular2">Nombre del Departamento</label>
													</div>
				...
				<div class="modal-dialog">
					<div class="modal-content" id='editaDeptoContent'>
						<div class="modal-header">
							...
							<h4 class="modal-title" id="formEditDeptoLabel">Modificar Departamento</h4>
						</div>
						<form class="form-horizontal" role="form">
							<div class="modal-body" id="editaDeptoBody">
								<div class="form-group">
									...
									<div class="col-sm-9">
										<input type="nombre" name="nombre" id="nomBanco" class="form-control" placeholder="Nombre del Departamento">
									</div>
								</div>
							</div>
		...
		$( document ).ready(function() {
			<?php
			if (isset( $_SESSION['ATZmsjAdminCatDeptos'])) {
				echo "notificaBad('".$_SESSION['ATZmsjAdminCatDeptos']."');";
				unset($_SESSION['ATZmsjAdminCatDeptos']);
			}
			if (isset( $_SESSION['ATZmsjSuccesAdminCatDeptos'])) {
				echo "notificaSuc('".$_SESSION['ATZmsjSuccesAdminCatDeptos']."');";
				unset($_SESSION['ATZmsjSuccesAdminCatDeptos']);
			}
			?>
		});
		...
		function formEditDepto(ident){
			...
			//alert('ID: '+ident);
			$.post("../funciones/formEditaDepto.php",
				{ ident:ident },
					function(respuesta){
