...
$info = new Seguridad();
$info->Acceso('catTiposVehiculos.php');
...
<html lang="es">
	...
	<body class="menubar-hoverable header-fixed me">
		...
		<div id="base">
								...
								<!-- BEGIN ACTION -->
										<div class="card card-bordered style-primary">
											<div class="card-head">
												...
												<header><i class="fa fa-fw fa-leaf"></i> Tipos de Vehiculos</header>
											</div><!--end .card-head -->
											<div class="card-body style-default-bright">
													...
													<tbody>
														...
														while ($dat = mysqli_fetch_array($res)) {
															...
															<tr '.$estatusText.'>
																<td>'.$count.'</td>
																<td>'.$dat['nombre'].'</td>
																<td class="col-lg-1 text-center '.$estatusColor.'" id="TipoVehiculoEstatus'.$dat['id'].'">'.$estatus.'</td>
																<td class="text-right">
																	<button type="button" class="btn btn-icon-toggle" onclick="formEditTipoVehiculo('.$dat['id'].')" data-toggle="modal" data-target="#formEditTipoVehiculo"><i class="fa fa-pencil" data-toggle="tooltip" data-placement="top" data-original-title="Editar Registro"></i></button>
																</td>
															</tr>';
														}
														?>
												...
												<br>
												<div class="col-lg-12">
													<form class="form" role="form" action="../funciones/registraNuevoCatTipoVehiculo.php" method="post">
														<div class="form-group floating-label col-lg-8">
															<input type="text" class="form-control" id="newTipoVehiculo" name="nombre">
															<label for="newTipoVehiculo">Nombre del TipoVehiculo</label>
														</div>
			...
			<div class="modal fade" id="formEditTipoVehiculo" tabindex="-1" role="dialog" aria-labelledby="formEditTipoVehiculoLabel" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content" id="editaTipoVehiculoContent">
					</div><!-- /.modal-content -->
				</div><!-- /.modal-dialog -->
			</div><!-- /.modal -->
		...
		$( document ).ready(function() {
			<?php
			if (isset( $_SESSION['ATZmsjAdminTipoVehiculo'])) {
				echo "notificaBad('".$_SESSION['ATZmsjAdminTipoVehiculo']."');";
				unset($_SESSION['ATZmsjAdminTipoVehiculo']);
			}
			if (isset( $_SESSION['ATZmsjSuccesAdminTipoVehiculo'])) {
				echo "notificaSuc('".$_SESSION['ATZmsjSuccesAdminTipoVehiculo']."');";
				unset($_SESSION['ATZmsjSuccesAdminTipoVehiculo']);
			}
			?>
		});

		function formEditTipoVehiculo(ident){
			//alert('ID: '+ident);
			$.post("../funciones/formEditaTipoVehiculo.php",
				{ ident:ident, pag:'<?=$info->pagina;?>' },
					function(respuesta){
						$("#editaTipoVehiculoContent").html(respuesta);
					});
		}
