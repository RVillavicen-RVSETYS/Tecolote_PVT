...
$info = new Seguridad();
$info->Acceso('catEspecialidades.php');
...
<html lang="es">
	...
	<body class="menubar-hoverable header-fixed me">
		...
		<div id="base">
								...
								<!-- BEGIN ACTION -->
										<div class="card card-bordered style-primary">
											<div class="card-head">
												...
												<header><i class="fa fa-star"></i> Especialidades</header>
											</div><!--end .card-head -->
											<div class="card-body style-default-bright">
												<?php
													require('../include/connect.php');
													$sql="SELECT *
																FROM catespecialidades
																ORDER BY nombre ASC";
												...
												<table class="table no-margin table-hover">
													...
													<tbody>
														...
														while ($dat = mysqli_fetch_array($res)) {
															...
															$estatusColor = ($dat['estatus'] == '1') ? 'text-success' : 'text-danger';
															$lineColor = ($dat['estatus'] == '1') ? '' : 'text-danger';
															...
															<tr class="'.$lineColor.'">
																...
																<td class="text-right">
																	<button type="button" class="btn btn-icon-toggle" onclick="formEditEspecialidades('.$dat['id'].')" data-toggle="modal" data-target="#formEditEspecialidades"><i class="fa fa-pencil" data-toggle="tooltip" data-placement="top" data-original-title="Editar Registro"></i></button>
																</td>
															</tr>';
														}
														?>
												...
												<div class="col-lg-12">
													<form class="form" role="form" action="../funciones/registraNuevoCatEspecialidades.php" method="post">
														<div class="form-group floating-label col-lg-8">
															<input type="text" class="form-control" id="newEspecialidades" name="nombre">
															<label for="newEspecialidades">Nombre de la Especialidad</label>
														</div>
														<div class="col-lg-4 text-center">
															<button type="submit" class="btn ink-reaction btn-raised btn-primary btn-loading-state" data-loading-text="<i class='fa fa-spinner fa-spin'></i> Cargando...">Agregar Especialidad</button>
														</div>
													</form>
												</div>
											</div><!--end .card-body -->
			...
			<div class="modal fade" id="formEditEspecialidades" tabindex="-1" role="dialog" aria-labelledby="formEditEspecialidadesLabel" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content" id="editaEspecialidadesContent">
					</div><!-- /.modal-content -->
				</div><!-- /.modal-dialog -->
			</div><!-- /.modal -->
		...
		$( document ).ready(function() {
			<?php
			if (isset( $_SESSION['ATZmsjAdminEspecialidades'])) {
				echo "notificaBad('".$_SESSION['ATZmsjAdminEspecialidades']."');";
				unset($_SESSION['ATZmsjAdminEspecialidades']);
			}
			if (isset( $_SESSION['ATZmsjSuccesAdminEspecialidades'])) {
				echo "notificaSuc('".$_SESSION['ATZmsjSuccesAdminEspecialidades']."');";
				unset($_SESSION['ATZmsjSuccesAdminEspecialidades']);
			}
			?>
		});

		function formEditEspecialidades(ident){
			$("#editaEspecialidadesBody").html('<span class="text-default"><img src="../assets/img/loading.gif" class="img-responsive" style="margin-bottom:-1em" alt="Cargando..."/></span>');
			//alert('ID: '+ident);
			$.post("../funciones/formEditaEspecialidades.php",
				{ ident:ident, pag:'<?=$info->pagina;?>' },
					function(respuesta){
						$("#editaEspecialidadesContent").html(respuesta);
					});
		}
