...
<html lang="es">
	...
	<body class="menubar-hoverable header-fixed me">
								...
								<div class="row">
									<div class="col-lg-12">
										...
										<?php
											require('../include/connect.php');
											$debug = 0;
											$sql = "SELECT MAX(placas) AS ultim FROM complementos WHERE estatus = '1' LIMIT 1";
											...
											$sql="SELECT cmk.nombre AS nomMarca, comp.tipo AS nomTipo, DATE_FORMAT(comp.fechaReg, '%d/%m/%Y %H:%i:%s') AS dateReg, comp.*,
														comp.idPoliza AS vPoliza,pol.contrato AS noContrato,ve.id AS idVe,ve.noEconomico AS noEco
											...
											<table id="datatable1" class="table table-striped table-hover">
												<thead>
													<tr>
														...
														<th class="text-center">Vehículo Asignado</th>
												...
												<tbody>
													...
													while ($dat = mysqli_fetch_array($res)) {
														...
														$fecha = $dat['dateReg'];
														if ( $dat['nomTipo'] == 1) {  $tipoComp= 'Góndola';	}
														else { $tipoComp= 'Dolly';	}

														echo '
														<tr '.$estatusText.'>
															<td><button type="button" class="btn ink-reaction btn-icon-toggle btn-primary" data-toggle="modal" data-target="#simpleModal" onclick="visualizaImg(\'../'.$foto.'\', \''.$tipoComp.'\', \''.$dat['placas'].'\');"><img class="img-circle width-1" src="../'.$foto.'" alt="IMG"></button></td>
															<td>'.$tipoComp.'</td>
															<td>'.$dat['placas'].'</td>
															<td>'.$dat['serie'].'</td>
															<td>'.$dat['nomMarca'].'</td>
				...
				<div class="modal-dialog">
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title" id="formNewUserLabel"><b>Crear Nuevo Complemento</b></h4>
						</div>
						<form class="form-horizontal" role="form" method="post" action="../funciones/registraNuevoComplemento.php" enctype="multipart/form-data">
	                  ...
	                  $resp3 = mysqli_query($link,$msql3) or die('<p class="text-danger">Notifica al Administrador</p>');
	                      ...
	                      echo '<option value="'.$dato3['id'].'">'.$dato3['contrato'].'</option>';
								...
								<div class="form-group">
									<div class="col-sm-3">
										<label for="vehiculo" class="control-label">Selecciona Vehículo a Asignar</label>
									</div>
									<div class="col-sm-9">
										<?php
										$vsql3 = "SELECT *
	                  ...
	                  $vresp3 = mysqli_query($link,$vsql3) or die('<p class="text-danger">Notifica al Administrador</p>');
										?>
										<select name="vehiculo" id="vehiculo" class="form-control" required>
											<?php
											while ($vdato3 = mysqli_fetch_array($vresp3)) {
	                      echo '<option value="'.$vdato3['id'].'">Eco - '.$vdato3['noEconomico'].'</option>';
											}
											?>
										</select>
									</div>
	  ...
	  $(document).ready(function(){
			...
			<?php
			if (isset( $_SESSION['ATZmsjEncargadoComplementos'])) {
				echo "notificaBad('".$_SESSION['ATZmsjEncargadoComplementos']."');";
				unset($_SESSION['ATZmsjEncargadoComplementos']);
			}
			if (isset( $_SESSION['ATZmsjSuccesEncargadoComplementos'])) {
				echo "notificaSuc('".$_SESSION['ATZmsjSuccesEncargadoComplementos']."');";
				unset($_SESSION['ATZmsjSuccesEncargadoComplementos']);
			}
			?>
	  });
		...
		function formEditoperador(ident){
			...
			//alert('ID: '+ident);
			$.post("../funciones/formEditaComplemento.php",
		...
		function verFoto(ident){
			$.post("../funciones/verFoto-Complemento.php",
				{ ident:ident},
					function(respuesta){
		...
		function visualizaImg(foto,tipo,placas){
			$("#cuerpoModal").html('<img class="height-8" width="100%" height="100%" src="'+foto+'">');
			$("#simpleModalLabel").html(tipo+" Placas: "+placas);
		}

		function eliminaFoto(ident){
			var tabla = ("complementos");
