								...
								<div class="col-lg-12">
									<div class="card-body">
										...
										$sql = "SELECT cmk.nombre AS nomMarca, ctv.nombre AS nomTipo, DATE_FORMAT(vhc.fechaReg, '%d/%m/%Y %H:%i:%s') AS dateReg, vhc.*,
											vhc.idPoliza AS vPoliza,pol.contrato AS noContrato, DATEDIFF(vhc.fechaVerificacion,NOW()) AS cantDias
														FROM vehiculos vhc
											...
											<table id="datatable1" class="table table-striped table-hover">
												...
												<tbody>
													...
													while ($dat = mysqli_fetch_array($res)) {
														...
														$estatusText = ($dat['estatus'] == 1) ? '' : 'text-danger';
														$foto = ($dat['foto'] == '') ? 'assets/img/noimg.png' : $dat['foto'];
														$fecha = $dat['dateReg'];
														$estatusVerificacion = ($dat['cantDias'] <= 10) ? 'style="background-color:#ffdc95;">' : '';
														$tag = ($dat['tag'] != '') ? $dat['tag'] : 'S/R';
														echo '
														<tr class = "' . $estatusText . '" ' . $estatusVerificacion . '>
								...
								<div class="col-sm-9 form">
									<div class="input-group date">
										<div class="input-group-content">
											<input type="text" class="form-control fechado" id="fechaVeri" name="fechaVeri" autocomplete="off" required>
										</div>
