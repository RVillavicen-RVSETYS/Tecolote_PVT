  	                 ...
  	                 <div class="form-group">
  	                  <div class="input-daterange input-group">
  	                   <div class="input-group-content">
  	                    	<input type="date" class="form-control fechado" name="fStart" autocomplete="off" value="<?=$fecha1;?>"/>
  	                      <label>Rango de Fechas</label>
  	                   </div>
  	                   <span class="input-group-addon">hasta</span>
  	                   <div class="input-group-content">
  	                      <input type="date" class="form-control fechado" name="fEnd"  autocomplete="off" value="<?=$fecha2;?>"/>
                ...
                <div class="card">
                  <div class="card-body no-padding">
                    ...
                    if ($fecha1 != '' AND $fecha2 !='') {
                      $filtrofecha = " AND DATE_FORMAT(carco.fechaReg,'%Y-%m-%d') BETWEEN '$fecha1' AND '$fecha2'";
                    }else {
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="card ">
                    ...
                    <?php
                    require('../include/connect.php');
                    $vsql2 = "SELECT carco.id,CONCAT(op.nombre,' ',op.apellidos) AS nomOpe, CONCAT('Eco ',ve.noEconomico,' Placas: ',ve.placas, ', ',ctpve.nombre,', ',cmrc.nombre,' ',csmrc.nombre) AS nomVehiculo,
                          ve.capacidadLts AS lts, (ve.capacidadLts - 200) AS ltsReal, date(carco.fechaReg) AS fechaCarga, carco.cant AS cantCargada, carco.kilometraje AS odometro
                          FROM cargacombustible carco
                          INNER JOIN viajes vjs ON vjs.id = carco.idViaje
                          INNER JOIN asignavehiculos asgv ON asgv.id = vjs.idAsignaVehiculo
                          INNER JOIN vehiculos ve ON ve.id = asgv.idVehiculo
                    ...
                    if ($operador != '') {
                      ...
                      while ($abc2=mysqli_fetch_array($vres2)){
                        ...
                        $muestraRendi = '';
                        $color = '';
                        $odoIN[$count] = $odoOut[$conta];
                        $odoOut[$count] = $abc2['odometro'];
                        $odometroIN = ($odoIN[$count] == '') ? $odoOut[$count] : $odoIN[$count] ;
                        $kmRecorrido = $odoOut[$count] - $odometroIN;
                        $rendAprox = $kmRecorrido * $rendEsperado;
                        $ltsEnTanque = $ltsEnTanqueReal[$conta] - ($rendEsperado * $kmRecorrido);
                        $ltsEnTanqueReal[$count] = $abc2['cantCargada'] + $ltsEnTanque;
                        $rendReal[$count] = $kmRecorrido / $abc2['cantCargada'];
                        $ltsUsados[$count] = $rendEsperado * $kmRecorrido;
                        $acumulador += $rendReal[$count];
                        $acumulador2 += $rendReal[$count];
                        $rendXfila = $kmRecorrido / $abc2['cantCargada'];

                        $datosGrafica2 .= "{ Operador: '".$abc2['nomOpe']."', Rendimiento: '".$rendXfila."' }, ";
                        $count++;
                        }
                      ...
                      <?php
                        if ($operador != '') {
                          ...
                          while ($rend = mysqli_fetch_array($res)) {
                            ...
                            $odometroIN = ($odoIN[$cont] == '') ? $odoOut[$cont] : $odoIN[$cont] ;
                            $kmRecorrido = $odoOut[$cont] - $odometroIN;
                            $rendAprox = $kmRecorrido * $rendEsperado;
                            $ltsEnTanque = $ltsEnTanqueReal[$conta] - ($rendEsperado * $kmRecorrido);
                            $ltsEnTanqueReal[$cont] = $rend['cantCargada'] + $ltsEnTanque;
                            $rendReal[$cont] = $kmRecorrido / $rend['cantCargada'];
                            $ltsUsados[$cont] = $rendEsperado * $kmRecorrido;
                            $acumulador += $rendReal[$cont];
                            $acumulador2 += $rendReal[$cont];
                            $rendXfila = $kmRecorrido / $rend['cantCargada'];
              ...
              <table id="datatable1" class="table table-striped table-hover">
                <thead>
                  <tr>
                    <th>#</th>
                    <th>Vehículo</th>
                    <th><center>Litros Entregados</center></th>
                  </tr>
                </thead>
                <tbody>
                <?php
