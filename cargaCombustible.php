...
$fechaInit = date("Y-m-01", strtotime($date));
$fechaFin = date("Y-m-t", strtotime($date));
?>
<!DOCTYPE html>
<html lang="es">
	<head>
	...
	<body class="menubar-hoverable header-fixed me">
												...
												<form id="filtrado">
													<div class="row border border-danger">
														...
														<div class="input-daterange input-group" id="rangoFechas">
															...
															<input type="text" class="form-control" name="fStart" 
															value="<?= $fechaInit; ?>" autocomplete="off" />
															</div>
															<span class="input-group-addon">hasta</span>
															<div class="input-group-content">
															<input type="text" class="form-control" name="fEnd" 
															value="<?= $fechaFin; ?>" autocomplete="off" />
														...
														<div class="col-md-5">
															...
															$res = mysqli_query($link, $sql) or die('<span class="text-danger">
															Por favor notifica al Administrador</span>')
															?>
															<div class="form-group">
																<label>Seleccione Operador</label>

																<select name="operador" id="operadorFiltro" class="form-control select2-list"
																data-placeholder="Select an item" value="0">
																	<option value=''>Todos los Operadores</option>
																	<optgroup label="Operadores">
																	<?php
																	while ($dat = mysqli_fetch_array($res)) {
																		$active = ($operador == $dat['id']) ? 'selected' : '';
																		echo '<option value="' . $dat['id'] . '" ' . $active . '> ' . 
																		$dat['nombre'] . ' ' . $dat['apellidos'] . ' </option>';
																	}
																	?>
																	</optgroup>
																</select>
														...
														<div class="col-md-2">
														<button type="submit" style="margin-top:5px;" class="btn ink-reaction
														btn-raised btn-primary btn-loading-state" 
														data-loading-text="<center>-- <i class='fa fa-spinner fa-spin'></i> --</center>">
														Buscar</button>

														</div>
													</div>
												</form>
												<div id="contentHistoriales">
											    </div>					
												</div><!--end .card-body -->
											</div><!--end .col -->
										</div><!--end .row -->
															 ...
															 <div class="card-body">
																 ...
																 <div class="form-group">
																	 ...
																	 <div class="col-sm-9">
																		 <div class="input-group">
																			 <div class="input-group-content">
																				 <input type="number" class="form-control" id="kilometros" step="0.1" min="0" name="kilometros" onkeyup="soloNumeros(this.value,'kilometros')" required>
																			 </div>
	  ...
	  $(document).ready(function(){
		...
		$("#contentHistoriales").load("templates/cargaTicketsCombustibles.php");
			...
			if (isset( $_SESSION['ATZmsjSuccesCargaCombustible'])) {
				$msn = explode('|' , $_SESSION['ATZmsjSuccesCargaCombustible'] );
				if ($msn[1] == 1) {
					$msj = $msn[2];
					$msj = str_replace("'" , "" , $msj);
					$msj = str_replace('"' , '' , $msj);
    			...
    			function(respuesta){
					//	alert('respuesta: '+respuesta);
						...
						if (res[0] == '1') {
							...
							$("#kilometros").attr({"value":res[3],"placeholder":"Debe ser mayor a: "+res[3]});
        ...
        $.ajax({
            type: 'POST',
            url: 'templates/cargaTicketsCombustibles.php',
            data: formData,
            success: function(respuesta) {
                $('#contentHistoriales').html(respuesta);

            },
            beforeSend: function() {
                $('#contentHistoriales').html('<div class="loading text-center"><img src="../assets/images/loading.gif"'+
				+' alt="loading" /><br/>Un momento, por favor...</div>');
            }
        });
    });
		</script>
